...
`debug` is included by the package; `jsonwebtoken` and `qs` are needed only by
the methods that use them. This helper does not load dotenv, preload databases, or resolve
